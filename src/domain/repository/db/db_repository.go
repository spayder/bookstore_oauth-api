...
func (r *dbRepository) GetById(accessTokenId string) (*access_token.AccessToken, *errors.RestErr) {
	...
	if err := cassandra.GetSession().Query(query, accessTokenId).Scan(
		&result.AccessToken,
		&result.UserId,
		&result.ClientId,
		&result.ExpiresAt,
...
func (r *dbRepository) Create(token access_token.AccessToken)  *errors.RestErr {
	...
	if err := cassandra.GetSession().Query(query, token.AccessToken, token.UserId, token.ClientId, token.ExpiresAt).Exec(); err != nil {
...
func (r *dbRepository) UpdateExpirationTime(token access_token.AccessToken) *errors.RestErr {
	...
	if err := cassandra.GetSession().Query(query, token.ExpiresAt, token.AccessToken).Exec(); err != nil {
		return errors.InternalServerError(err.Error())
	}
	return nil
}